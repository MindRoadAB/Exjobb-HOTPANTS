...
struct Image {
  ...
  Image(const std::string n, const std::string p = "res/", std::valarray<double> v = {}, std::pair<long, long> a = {0L,0L}) : name{n}, path{p}, data{v} , axis{a} {}
  Image(const Image& other) : name{other.name}, path{other.path}, data{other.data}, axis{other.axis} {}
  Image(Image&& other) : name{other.name}, path{other.path}, data{other.data}, axis{other.axis} {}
...
int writeImage(Image& img) {
  ...
  long nEl = std::accumulate(&img.axis.first, &img.axis.second, 1, std::multiplies<long>());
  long fpixel(1);
